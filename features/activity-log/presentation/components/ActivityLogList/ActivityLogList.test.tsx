...
const entry = (id: string, person: ActivityLogEntry["person"], date: string): ActivityLogEntry => ({
  id,
  date,
  person,
...
describe("ActivityLogList — today renders flat list", () => {
  it("renders flat ActivityLogEntryCards for today's entries", () => {
    const entries = [entry("1", "Meme", TODAY), entry("2", "Pedro", TODAY)];
    render(<ActivityLogList entries={entries} onDelete={vi.fn()} selectedMonth="2026-06" />);
...
describe("ActivityLogList — past day renders person groups alphabetically", () => {
  it("renders two person groups for Meme and Pedro", () => {
    const entries = [entry("1", "Pedro", YESTERDAY), entry("2", "Meme", YESTERDAY)];
    render(<ActivityLogList entries={entries} onDelete={vi.fn()} selectedMonth="2026-06" />);
...
describe("ActivityLogList — past day with single person", () => {
  it("renders only one person group when only Pedro has entries", () => {
    const entries = [entry("1", "Pedro", YESTERDAY), entry("2", "Pedro", YESTERDAY)];
    render(<ActivityLogList entries={entries} onDelete={vi.fn()} selectedMonth="2026-06" />);
    const headings = document.querySelectorAll("h4");
