...
// read action for either of those two, since the page-level redirect already gates access
// before the loader runs. `handleLoadTransactions` is the exception: the client hook calls
// it directly on every month change, so it is POST-reachable on its own and gates auth
// itself.
