...
type FormState = { type: FinanceEntry["type"]; group: string; category: string; amount: string; date: string; description: string };

const toForm = (e: FinanceEntry): FormState => ({
  type: e.type, group: e.group, category: e.category,
  amount: String(e.amount), date: e.date, description: e.description ?? "",
});
...
export function EditEntryModal({ entry, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    ...
    if (!dialog) return;
    if (entry) { setForm(toForm(entry)); dialog.showModal(); }
    else dialog.close();
  }, [entry]);
  ...
  const handleGroupChange = (group: string) => {
    setForm((f) => f ? { ...f, group, category: "" } : f);
  };
