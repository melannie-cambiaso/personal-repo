  ...
  return (
    ...
    >
      <div className="px-6 py-5">
        <h2 className="font-dancing mb-3 text-2xl font-bold text-brown-900">¿Eliminar zona?</h2>
        <p className="mb-5 text-sm leading-relaxed text-brown-600">
          La zona <strong>{zone?.emoji ? `${zone.emoji} ` : ""}{zone?.name}</strong> tiene{" "}
          <strong>{itemCount} {itemCount === 1 ? "mejora" : "mejoras"}</strong>.
        ...
        <div className="flex justify-end gap-3">
          ...
          <button type="button" onClick={onConfirm} className={btnDanger}>Eliminar todo</button>
        </div>
      </div>
    </dialog>
  );
