...
describe("useWishlist", () => {
  it("returns initial items and owned ids", () => {
    ...
    const { result } = renderHook(() =>
      useWishlist({ initialItems: items, initialOwnedIds: ["1"], onAdd: vi.fn(), onToggle: vi.fn() })
    )
  ...
  it("toggle adds id to ownedIds and updates pending", () => {
    ...
    const { result } = renderHook(() =>
      useWishlist({ initialItems: [makeItem("1"), makeItem("2")], initialOwnedIds: [], onAdd: vi.fn(), onToggle })
    )
  ...
  it("toggle removes id when already owned", () => {
    const { result } = renderHook(() =>
      useWishlist({ initialItems: [makeItem("1")], initialOwnedIds: ["1"], onAdd: vi.fn(), onToggle: vi.fn() })
    )
    act(() => result.current.toggle("1"))
  ...
  it("totalPrice sums pending items with non-null price", () => {
    ...
    const { result } = renderHook(() =>
      useWishlist({ initialItems: items, initialOwnedIds: ["2"], onAdd: vi.fn(), onToggle: vi.fn() })
    )
